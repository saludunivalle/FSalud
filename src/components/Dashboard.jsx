...
const StatusChip = ({ status, onClick, highlight }) => {
  ...
  return (
    <Chip
      ...
      sx={{
        ...
        fontSize: '0.75rem',
        height: 28,
        boxShadow: highlight ? '0 0 0 3px #f4433622' : undefined,
        animation: highlight ? 'pulse 1.2s infinite' : undefined,
        cursor: onClick ? 'pointer' : 'default',
        '& .MuiChip-icon': {
          color: color,
        },
        '@keyframes pulse': {
          '0%': { boxShadow: '0 0 0 0 #f4433622' },
          '70%': { boxShadow: '0 0 0 6px #f4433600' },
          '100%': { boxShadow: '0 0 0 0 #f4433622' },
        },
      }}
    />
  );
                    ...
                    {filteredDocuments.length === 0 ? (
                      ...
                      filteredDocuments.map((doc, index) => [
                        ...
                        >
                            ...
                            {doc.status?.toLowerCase() === 'aprobado' || doc.status?.toLowerCase() === 'cumplido' ? (
                              <Button
                                ...
                                sx={{
                                  ...
                                  minWidth: '110px',
                                  fontSize: '0.75rem',
                                  background: 'linear-gradient(135deg,rgb(169, 169, 169) 0%,rgb(133, 181, 99) 100%)',
                                  color: 'white',
                                  opacity: 0.7,
                                  cursor: 'not-allowed',
                                  '&:hover': {
                                    background: 'linear-gradient(135deg,rgb(57, 57, 57) 0%,rgb(255, 64, 89) 100%)',
                                  }
                                }}
                              >
                                Aprobado
                          ...
                          <TableCell>
                            {doc.status?.toLowerCase() === 'rechazado' && doc.userDocData?.comentario ? (
                              <StatusChip
                                status={doc.status}
                       ...
                       // Fila expandible para mostrar observaciones de rechazo
                       doc.status?.toLowerCase() === 'rechazado' && doc.userDocData?.comentario && (
                         <TableRow key={`obs-${index}`}>
                           <TableCell colSpan={8} sx={{ p: 0, background: '#fff8e1' }}>
                             <Collapse in={expandedRow === index} timeout="auto" unmountOnExit>
                               <Box sx={{ p: 2, display: 'flex', alignItems: 'flex-start', gap: 2 }}>
                                 <InfoIcon color="error" sx={{ mt: 0.5 }} />
                                 <Box>
                                   <Typography variant="subtitle2" color="error" sx={{ fontWeight: 700 }}>
                                     Motivo del rechazo
                                   </Typography>
                                   <Typography variant="body2" color="text.secondary">
                                     {doc.userDocData.comentario}
                                   </Typography>
                                 </Box>
                               </Box>
                             </Collapse>
